	...
	public MulithreadedStringServerExampleConsole()
	{
		try
		{
			...
			while (true)
			{
				...
				//store the local client number, but increment it first before storing it
				int localClientNumber = ++clientNumber;
				
				// Display client connection
				InetAddress inetAddress = socket.getInetAddress();
				System.out.println("\n@@@ New Connection @@@\nStarting new thread for client number " + localClientNumber);
				...
				new Thread(new Runnable()
				{
					...
					public void run()
					{
						try
						{
							...
							// Send MOTD
							dataForClient.writeUTF("\nMOTD: The survey says.. I mean the server says: Hello client " + localClientNumber + ".");
							dataForClient.flush();
							...
							while (true)
							{
								...
								// Send ACK to client
								dataForClient.writeUTF("*Red Alert Russian Accent* Acknowledged transmission of - '" + clientInput + "' from client " + localClientNumber);
								dataForClient.flush();
							}
						} catch (EOFException eof)
						{
							System.out.println("\nClient " + localClientNumber + " has disconnected.");
