...
public class SupportSystem
{
    ...
    public void start()
    {
        ...
        while(!finished) {
            ...
            //ex 5.8 is using trim() to remove space
            //ex 5.9 is using toLowerCase() to convert to lowercase
            input = input.trim().toLowerCase();

            //if(input.startsWith("bye")) {
            //ex 5.11 is using .equals instead of startsWith
            if(input.equals("bye")) {
                finished = true;
            }
            else {
                //String response = responder.generateResponseArrayList();
                //String response = responder.getResponseHashMap(input);
                String response = responder.getResponseHashMap2(input);
